          ...
          <form method="get">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Field
                label="Actor email"
                htmlFor="actor"
                hint="Matches the email recorded on the row."
              >
                <Input
                  id="actor"
                  name="actor"
                  defaultValue={filters.actor}
                  placeholder="name@bajajlife.com"
                />
              </Field>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              {hasFilters ? (
                <LinkButton href="/admin/audit" variant="ghost">
                  Clear
                </LinkButton>
              ) : null}
            </div>
          ...
          <>
            <Table>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id}>
                    <Td className="font-mono text-[11px] whitespace-nowrap text-slate-500">
                      {formatDateTime(row.createdAt)}
                    </Td>
                    <Td>
                      <span className="block break-all text-[12px]">{orDash(row.actorEmail)}</span>
                      {row.actorRole ? (
                    ...
                    <Td className="font-mono text-[12px] whitespace-nowrap">{row.action}</Td>
                    <Td>
                      <span className="block text-[11px] text-slate-500">{row.entityType}</span>
